...
drop function public.save_run_with_checkin(timestamp with time zone, timestamp with time zone, integer, numeric, integer, jsonb, text, numeric, boolean, uuid);
CREATE OR REPLACE FUNCTION public.save_run_with_checkin(p_started_at timestamp with time zone, p_ended_at timestamp with time zone, p_duration_seconds integer, p_distance_miles numeric, p_pace_sec integer, p_points jsonb, p_event_type text, p_goal_miles numeric DEFAULT NULL::numeric, p_goal_hit boolean DEFAULT NULL::boolean, p_client_run_id uuid DEFAULT NULL::uuid, p_avg_heart_rate integer DEFAULT NULL::integer, p_source text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
